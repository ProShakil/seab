...
watch(search, (value) => {
  router.get('/admin/breaking-news', { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const openEdit = (item) => {
  editMode.value = true
  form.value = { id: item.id, title: item.title }
  modalOpen.value = true
}
...
const save = () => {
  if (editMode.value) {
    router.put(`/admin/breaking-news/${form.value.id}`, form.value, {
      onSuccess: () => {
    ...
    router.post('/admin/breaking-news', form.value, {
      onSuccess: () => {
        modalOpen.value = false
...
const toggleStatus = (item) => {
  router.put(route('admin.breaking-news.toggle', item.id), {
...
const destroy = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(`/admin/breaking-news/${id}`, {
...
<template>
  <AdminLayout>
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <h1 class="text-xl font-bold">Breaking News</h1>
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center">
      ...
      <div class="bg-white p-6 rounded w-[400px]">
        ...
        <h2 class="text-lg font-bold mb-4">
          {{ editMode ? 'Edit' : 'Add' }} Breaking News
        </h2>
