...
watch(search, (value) => {
  router.get('/admin/membership-type', { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const save = () => {
  if (editMode.value) {
    router.put(`/admin/membership-type/${form.value.id}`, form.value, {
      onSuccess: () => {
    ...
    router.post('/admin/membership-type', form.value, {
      onSuccess: () => {
        modalOpen.value = false
...
const toggleStatus = (item) => {
  router.put(route('admin.membership-type.toggle', item.id), {
...
const destroy = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(`/admin/membership-type/${id}`, {
...
<template>
  <AdminLayout>
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <h1 class="text-xl font-bold">Membership Type</h1>
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center">
      ...
      <div class="bg-white p-6 rounded w-[400px]">
        ...
        <h2 class="text-lg font-bold mb-4">
          {{ editMode ? 'Edit' : 'Add' }} Membership Type
        </h2>
