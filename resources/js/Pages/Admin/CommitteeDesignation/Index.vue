...
watch(search, (value) => {
  router.get('/admin/committee-designation', { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const save = () => {
  if (editMode.value) {
    router.put(`/admin/committee-designation/${form.value.id}`, form.value, {
      onSuccess: () => {
    ...
    router.post('/admin/committee-designation', form.value, {
      onSuccess: () => {
        modalOpen.value = false
...
const toggleStatus = (item) => {
  router.put(route('admin.committee-designation.toggle', item.id), {
...
const destroy = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(`/admin/committee-designation/${id}`, {
...
<template>
  <AdminLayout>
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <h1 class="text-xl font-bold">Committee Designation</h1>
    ...
    <table class="w-full border">
      ...
      <tbody>
        <tr v-for="(item,index) in data.data" :key="item.id">
          ...
          <!-- ACTION ICONS -->
          <td class="p-2 border flex gap-3 text-center">
    ...
    <div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center">
      ...
      <div class="bg-white p-6 rounded w-[400px]">
        ...
        <h2 class="text-lg font-bold mb-4">
          {{ editMode ? 'Edit' : 'Add' }} Committee Designation
        </h2>
