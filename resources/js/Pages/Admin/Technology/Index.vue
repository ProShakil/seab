...
watch(search, (value) => {
  router.get('/admin/technology', { search: value }, {
    preserveState: true,
    replace: true
  })
})
...
const save = () => {
  if (editMode.value) {
    router.put(`/admin/technology/${form.value.id}`, form.value, {
      onSuccess: () => {
    ...
    router.post('/admin/technology', form.value, {
      onSuccess: () => {
        modalOpen.value = false
...
const toggleStatus = (item) => {
  router.put(route('admin.technology.toggle', item.id), {
...
<template>
  <AdminLayout>
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <h1 class="text-xl font-bold">Technology</h1>
